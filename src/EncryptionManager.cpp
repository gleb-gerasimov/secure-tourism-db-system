...
QByteArray EncryptionManager::encrypt(const QString& plainText) {
    ...
    EVP_EncryptUpdate(
        ...
        &length,
        reinterpret_cast<const unsigned char*>(plainData.data()),
        plainData.size()
    );
    ...
    EVP_EncryptFinal_ex(
        context,
        reinterpret_cast<unsigned char*>(encryptedData.data()) + totalLength,
        &length
    );
...
QString EncryptionManager::decrypt(const QByteArray& encryptedData) {
    ...
    EVP_DecryptFinal_ex(
        context,
        reinterpret_cast<unsigned char*>(decryptedData.data()) + totalLength,
        &length
    );
...
QByteArray EncryptionManager::key() {
